	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if directUpload {
				...
				if err := deployEncodersIfConfigured(cmd.Context(), cfg, rootDir, directUpload, workers); err != nil {
					return err
				}
				return deployReadersIfConfigured(cmd.Context(), cfg, rootDir, directUpload, workers)
			}
...
func deployReadersIfConfigured(ctx context.Context, cfg Config, rootDir string, directUpload bool, workers int) error {
	...
	if directUpload {
		if err := deployPayloadDirect(cfg.Readers, readerTarPath, "/root", "reader-payload/reader_init.sh", 7*time.Minute, workers); err != nil {
			return fmt.Errorf("reader deployment: %w", err)
		}
	} else {
		if err := deployPayloadViaS3(ctx, rootDir, cfg.Readers, readerTarPath, "/root", "reader-payload/reader_init.sh", 7*time.Minute, cfg.S3Config, workers); err != nil {
			return fmt.Errorf("reader deployment: %w", err)
		}
	}
