...
func fibreTxsimCmd() *cobra.Command {
	...
	cmd.Flags().BoolVar(&uploadOnly, "upload-only", false, "skip PFF transaction entirely — only upload shards to validators (no MsgPayForFibre broadcast, fibre-reader will see nothing). Default false: PFF is broadcast async (mempool submit only; confirmation tracked by background workers, doesn't block uploads)")
