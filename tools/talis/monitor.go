...
'talis download-resources' to collect the results.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if stop {
				...
				return stopTmuxSession(validators, ResourceMonitorSessionName, 5*time.Minute)
			}
			...
			if err := runScriptInTMux(validators, script, ResourceMonitorSessionName, 5*time.Minute); err != nil {
